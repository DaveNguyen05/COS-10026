<!DOCTYPE html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <meta name="description" content="About page">
  <meta name="keywords" content="Assignment1 about">
  <meta name="author" content="DGD">
  <link href="styles/style.css" rel="stylesheet">
  <title>Assignment2 about page</title>
</head>
<div id="container">

  <header>
    <?php include './inc/header.inc'; ?>
    <?php include './inc/menu.inc'; ?>
  </header>

  <div class="bg-image"></div>
  <i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i><i class="rain"></i>

  <section class="about-header">
    <h2>About Us</h2>
    <p>Team DGD is a group of students, there is virtually nothing interesting about them that can be communicated via this website. However, in the interest of filling whitespace and fulfilling the design requirements, we are proud to present a handful of extremely brief bio's, and realistically like one timetable from the guy that volunteered to make it. Enjoy.</p>
  </section>

  <div class="about-definition-description-list"> <!-- Not sure what w3 validator is complaining about here -->
    <img id="group-image" src="./images/groupPhoto.jpg" alt="Image">
    <dl>
      <dt>Dillan Carroll</dt>
      <dd>Group ID - 101617498</dd>
      <dt>Bio</dt>
      <dd>Founder of up to five fortune 500 companies, may or may not have made this page.</dd>
      <dt>Email</dt><a href="mailto:101617498@student.swin.edu.au">101617498@student.swin.edu.au</a>
    </dl>

    <dl>
      <dt>Gary HO</dt>
      <dd>Group ID - 104052445</dd>
      <dt>Bio</dt>
      <dd>Gary wears a jacket sometimes, and wears glasses.</dd>
      <dt>Email</dt><a href="mailto:104052445@student.swin.edu.au">104052445@student.swin.edu.au</a>
    </dl>

    <dl>
      <dt>Dave Nguyen</dt>
      <dd>Group ID - 104697710</dd>
      <dt>Bio</dt>
      <dd>Often known to be in the foreground of group photos.</dd>
      <dt>Email</dt><a href="104697710@student.swin.edu.au">104697710@student.swin.edu.au</a>
    </dl>
  </div>

  <section id="about-timetable-container">
    <h2>Timetable</h2>
    <table class="Timetable">
      <thead>
        <tr>
          <th></th>
          <th>Monday&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
          <th>Tuesday&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
          <th>Wednesday</th>
          <th>Thursday</th>
          <th>Friday&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
          <th>Saturday</th>
          <th>Sunday</th>
        </tr>
      </thead>
      <tbody>
        <tr>
          <td>6 am</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>7 am</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>8 am</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td><img id="fri830" src="./images/fri830.png" alt="Image"></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>9 am</td>
          <td><img id="mon930" src="./images/mon930.png" alt="Image"></td>
          <td></td>
          <td></td>
          <td></td>
          <td><img id="fri930" src="./images/fri930.png" alt="Image"></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>10 am</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>11 am</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>Noon</td>
          <td><img id="mon1230" src="./images/mon1230.png" alt="Image"></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>1 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>2 pm</td>
          <td><img id="mon230" src="./images/mon230.png" alt="Image"></td>
          <td></td>
          <td></td>
          <td></td>
          <td><img id="fri230" src="./images/fri230.png" alt="Image"></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>3 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>4 pm</td>
          <td></td>
          <td><img id="tue430" src="./images/tue430.png" alt="Image"></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>5 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>6 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>7 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>8 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>9 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
        <tr>
          <td>10 pm</td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
          <td></td>
        </tr>
      </tbody>
    </table>
  </section>
  <br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
  <?php include './inc/footer.inc'; ?>
</div>

</html>